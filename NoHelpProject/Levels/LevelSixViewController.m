...
@interface LevelSixViewController ()
@property (nonatomic) LevelsModelManager *levelsModelManager;
@end

@implementation LevelSixViewController
...
- (IBAction)helpMessage:(id)sender {
    NSString *title = @"HELP";
    NSString *message = @"Feeling hungry?";
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:title message:message preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *okButton = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [alert dismissViewControllerAnimated:YES completion:nil];
    }];
...
-(void)endLevelAnimation{
    [self.heroImageView setImage:[UIImage imageNamed:@"ultron"]];
...
-(void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event{
    if(event.subtype == UIEventSubtypeMotionShake){
        self.appleTreeImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"appleTreeEmpty"]];
        
    }
}
- (IBAction)checkButton:(id)sender {
    if([self.pickerViewInfo.inputKey isEqualToString: @"2340"]){
        [self.checkImageButton setImage:[UIImage imageNamed:@"buttonGreen"] forState:UIControlStateNormal];
        [self endLevelAnimation];
        [self.levelsModelManager insertWithLevelId:6 levelCompleted:YES];
